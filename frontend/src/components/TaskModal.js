...
const TaskModal = ({ isOpen, setIsOpen, id }) => {
    const [taskData, setTaskData] = useState(null); // Initialize taskData as null
    ...
    useEffect(() => {
        const fetchTaskDetails = async () => {
            try {
                const response = await axios.get(`http://localhost:9000/project/${id.projectId}/task/${id.id}`);
                            ...
                            <Dialog.Panel className="rounded-md bg-white max-w-[85%] w-[85%] h-[85%] overflow-y-hidden">
                                ...
                                <div className='flex gap-4 h-[inherit]'>
                                    <div className="!w-8/12 px-8 space-y-3 py-4 min-h-max  overflow-y-auto">
                                        {taskData ? (
                                            <>
                                                <h1 className='text-3xl font-semibold '>Title: {capitalizeFirstLetter(taskData.title)}</h1>
                                                <p className='text-gray-600'>Description :{capitalizeFirstLetter(taskData.description)}</p>
                                                <p className='text-gray-600'>Status: {taskData.status ? 'Completed' : 'Pending'}</p>
                                                <p className='text-gray-600'>Date: {formatDate(taskData.createdDate)}</p>
                                            </>
                                        ) : (
                                            <p className="text-red-500">Task details not found</p>
                                        )}
                                    </div>
